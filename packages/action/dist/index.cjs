...
function filterFiles(files, maxFiles) {
...
function prepareDiffs(files, maxFiles, maxDiffBytes) {
    ...
    async listModels() {
      try {
        ...
        return data.data.map((m) => ({
          ...
          created: m.created_at ? new Date(m.created_at).getTime() / 1e3 : void 0
...
function getAnthropicFallbackModels() {
  return [
    ...
    async listModels() {
      try {
        ...
        return data.models.map((m) => ({
          id: m.name,
          name: m.name,
          created: new Date(m.modified_at).getTime() / 1e3,
...
async function getBotLogin(octokit) {
  try {
